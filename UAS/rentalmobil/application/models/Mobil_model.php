...
class mobil_model extends CI_Model{
    ...
    public function getMobil($id = null){
        if($id == null){
            ...
            $this->db->select("id_mobil,mobil.id_type as id_type, type.nama_type as type, merk, no_plat, warna, tahun, harga, if(status=1, 'Tersedia', 'Disewa') as status ,CONCAT('$base', gambar) as gambar");
            $this->db->from('mobil');
            $this->db->join('type', 'mobil.id_type = type.id_type');
